...
pub fn bench_crc8_no_lut(c: &mut Criterion) {
    ...
    const CRC: Crc<NoLookupTable> = Crc::<NoLookupTable>::new(&CRC8_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC8_PARAMS.check);
    ...
    for size in CHUNK_SIZES.iter() {
        ...
        group.bench_function(BenchmarkId::new("NoLookupTable", *size), |b| {
            b.iter(|| black_box(CRC.compute(&bytes)))
        });
    }
...
pub fn bench_crc8_lut_32(c: &mut Criterion) {
    ...
    const CRC: Crc<LookupTable32> = Crc::<LookupTable32>::new(&CRC8_PARAMS);
...
pub fn bench_crc8_lut_256(c: &mut Criterion) {
    ...
    const CRC: Crc<LookupTable256> = Crc::<LookupTable256>::new(&CRC8_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC8_PARAMS.check);
...
pub fn bench_crc8_lut_256x_n(c: &mut Criterion) {
    ...
    const CRC: Crc<LookupTable256xN<SLICES>> = Crc::<LookupTable256xN<SLICES>>::new(&CRC8_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC8_PARAMS.check);
...
pub fn bench_crc16_no_lut(c: &mut Criterion) {
    ...
    const CRC: Crc<NoLookupTable> = Crc::<NoLookupTable>::new(&CRC16_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC16_PARAMS.check);
...
pub fn bench_crc16_lut_32(c: &mut Criterion) {
    ...
    const CRC: Crc<LookupTable32> = Crc::<LookupTable32>::new(&CRC16_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC16_PARAMS.check);
...
pub fn bench_crc16_lut_256(c: &mut Criterion) {
    ...
    const CRC: Crc<LookupTable256> = Crc::<LookupTable256>::new(&CRC16_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC16_PARAMS.check);
...
pub fn bench_crc16_lut_256x_n(c: &mut Criterion) {
    ...
    const CRC: Crc<LookupTable256xN<SLICES>> = Crc::<LookupTable256xN<SLICES>>::new(&CRC16_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC16_PARAMS.check);
...
pub fn bench_crc32_no_lut(c: &mut Criterion) {
    ...
    const CRC: Crc<NoLookupTable> = Crc::<NoLookupTable>::new(&CRC32_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC32_PARAMS.check);
...
pub fn bench_crc32_lut_32(c: &mut Criterion) {
    ...
    const CRC: Crc<LookupTable32> = Crc::<LookupTable32>::new(&CRC32_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC32_PARAMS.check);
...
pub fn bench_crc32_lut_256(c: &mut Criterion) {
    ...
    const CRC: Crc<LookupTable256> = Crc::<LookupTable256>::new(&CRC32_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC32_PARAMS.check);
...
pub fn bench_crc32_lut_256x_n(c: &mut Criterion) {
    ...
    const CRC: Crc<LookupTable256xN<SLICES>> = Crc::<LookupTable256xN<SLICES>>::new(&CRC32_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC32_PARAMS.check);
...
pub fn bench_crc64_no_lut(c: &mut Criterion) {
    ...
    const CRC: Crc<NoLookupTable> = Crc::<NoLookupTable>::new(&CRC64_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC64_PARAMS.check);
...
pub fn bench_crc64_lut_32(c: &mut Criterion) {
    ...
    const CRC: Crc<LookupTable32> = Crc::<LookupTable32>::new(&CRC64_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC64_PARAMS.check);
...
pub fn bench_crc64_lut_256(c: &mut Criterion) {
    ...
    const CRC: Crc<LookupTable256> = Crc::<LookupTable256>::new(&CRC64_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC64_PARAMS.check);
...
pub fn bench_crc64_lut_256x_n(c: &mut Criterion) {
    ...
    const CRC: Crc<LookupTable256xN<SLICES>> = Crc::<LookupTable256xN<SLICES>>::new(&CRC64_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC64_PARAMS.check);
...
pub fn bench_crc128_no_lut(c: &mut Criterion) {
    ...
    const CRC: Crc<NoLookupTable> = Crc::<NoLookupTable>::new(&CRC128_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC128_PARAMS.check);
...
pub fn bench_crc128_lut_32(c: &mut Criterion) {
    ...
    const CRC: Crc<LookupTable32> = Crc::<LookupTable32>::new(&CRC128_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC128_PARAMS.check);
...
pub fn bench_crc128_lut_256(c: &mut Criterion) {
    ...
    const CRC: Crc<LookupTable256> = Crc::<LookupTable256>::new(&CRC128_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC128_PARAMS.check);
...
pub fn bench_crc128_lut_256x_n(c: &mut Criterion) {
    ...
    const CRC: Crc<LookupTable256xN<SLICES>> = Crc::<LookupTable256xN<SLICES>>::new(&CRC128_PARAMS);
    assert_eq!(CRC.compute(SANITY_CHECK_DATA), CRC128_PARAMS.check);
