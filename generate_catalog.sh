...
curl -s https://reveng.sourceforge.io/crc-catalogue/all.htm | grep -o 'width.*name.*"' | while read -r line; do
  ...
  if [ $width -le 8 ]; then
    echo "pub const $name: Params<u8> = Params { width: $width, $params };"
  elif [ $width -le 16 ]; then
    echo "pub const $name: Params<u16> = Params { width: $width, $params };"
  elif [ $width -le 32 ]; then
    echo "pub const $name: Params<u32> = Params { width: $width, $params };"
  elif [ $width -le 64 ]; then
    echo "pub const $name: Params<u64> = Params { width: $width, $params };"
  elif [ $width -le 128 ]; then
    echo "pub const $name: Params<u128> = Params { width: $width, $params };"
  fi
done
