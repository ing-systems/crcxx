...
imp_make_sliced_lut!(crc32_make_sliced_lut, u32, crc32_make_lut_256);
...
pub fn crc32_update_slice_by<const SLICES: usize, const REFLECT: bool>(
...
fn update_slice_by_4<'a, const REFLECT: bool>(
    mut crc: u32, mut bytes: &'a [u8], lut: &[[u32; 256]],
) -> (u32, &'a [u8]) {
    const STEP: usize = 4;
...
mod tests {
    ...
    fn with_reflect() {
        ...
        let lut256x_n = crc32_make_sliced_lut::<SLICES>(POLY, REFLECT);
        ...
        for sample in SAMPLES {
            ...
            assert_eq!(
                crc32_update_slice_by::<SLICES, REFLECT>(INIT, sample.0.as_bytes(), &lut256x_n)
                    ^ XOR_OUT,
                sample.1
            );
        }
