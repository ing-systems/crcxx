...
imp_crc_update_lut_32!(crc16_update_lut_32, u16);
imp_crc_update_ref_lut_32!(crc16_update_ref_lut_32, u16);

imp_crc_update_lut_256!(crc16_update_lut_256, u16);
imp_crc_update_ref_lut_256!(crc16_update_ref_lut_256, u16);

imp_make_sliced_lut!(crc16_make_sliced_lut, u16, crate::lut::crc16_make_lut_256);

pub fn crc16_update(mut crc: u16, mut buf: &[u8], lut: &[[u16; 256]]) -> u16 {
    // slice-by-32
    if SLICES >= 32 && lut.len() >= SLICE_32 && buf.len() >= SLICE_32 {
        while buf.len() >= SLICE_32 {
            crc = lut[0x1f][buf[0x00] as usize ^ ((crc >> 0x08) & 0xFF) as usize]
                ^ lut[0x1e][buf[0x01] as usize ^ (crc & 0xFF) as usize]
                ^ lut[0x1d][buf[0x02] as usize]
                ^ lut[0x1c][buf[0x03] as usize]
    ...
    if SLICES >= 16 && lut.len() >= SLICE_16 && buf.len() >= SLICE_16 {
        while buf.len() >= SLICE_16 {
            crc = lut[0xf][buf[0x0] as usize ^ ((crc >> 0x08) & 0xFF) as usize]
                ^ lut[0xe][buf[0x1] as usize ^ (crc & 0xFF) as usize]
                ^ lut[0xd][buf[0x2] as usize]
                ^ lut[0xc][buf[0x3] as usize]
    ...
    if SLICES >= 8 && lut.len() >= SLICE_8 && buf.len() >= SLICE_8 {
        while buf.len() >= SLICE_8 {
            crc = lut[0x7][buf[0x0] as usize ^ ((crc >> 0x08) & 0xFF) as usize]
                ^ lut[0x6][buf[0x1] as usize ^ (crc & 0xFF) as usize]
                ^ lut[0x5][buf[0x2] as usize]
                ^ lut[0x4][buf[0x3] as usize]
    ...
    if SLICES >= 4 && lut.len() >= SLICE_4 && buf.len() >= SLICE_4 {
        while buf.len() >= SLICE_4 {
            crc = lut[0x3][buf[0x0] as usize ^ ((crc >> 0x08) & 0xFF) as usize]
                ^ lut[0x2][buf[0x1] as usize ^ (crc & 0xFF) as usize]
                ^ lut[0x1][buf[0x2] as usize]
                ^ lut[0x0][buf[0x3] as usize];
